...
function UpdateProduct() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (productSnapshot.data()) {
          reset(productSnapshot.data());
          setImgUrl(getValues("image"));
          setCurrentPrice(getValues("price"));
          setListImage(getValues("images"));
          setListColor(getValues("colors"));
          setListSize(getValues("sizes"));
          setContent(getValues("descriptions") || "");
        }
      } catch (err) {
        console.log(err);
      }
  ...
  const onSubmitAddNewProduct = async (values) => {
    ...
    try {
      ...
      const cloneValuesPost = {
        ...
        old_price:
          Number(currentPrice) !== Number(values.price)
            ? Number(currentPrice)
            : Number(values.price),
  ...
  return (
    <div className={cx("wrapper")}>
      {loading && <Loading />}
      <Heading>Chỉnh sửa sản phẩm</Heading>
